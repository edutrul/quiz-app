...
![CI](https://github.com/edutrul/quiz-app/actions/workflows/ci.yml/badge.svg)
...
See `CLAUDE.md` for the fuller design rationale (attempt identity, idempotency via a DB unique constraint, TanStack Query for server state) and project conventions.
...
```bash
npm test -w server      # node:test + supertest, against an in-memory SQLite db
npm run build -w client # typecheck + build (no client test suite yet)
```
...
- `server/` — Express API + SQLite (`better-sqlite3`), raw SQL via `schema.sql`, no ORM. `src/app.js` exports the testable Express app; `src/index.js` starts the listener.
